...
| LoCoMo | https://github.com/snap-research/locomo | CC BY-NC 4.0 upstream; verify before use | English | long-term dialog memory | synthetic LLM-agent conversations with annotations | local file kept outside git under ignored `data/` | Tests long multi-session conversational QA, temporal and multi-hop recall outside Engram's hand-authored suites | not a governance/GDPR benchmark; images ignored; license is non-commercial | Local text-only run completed |
