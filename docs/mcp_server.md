...
| Tool | Purpose |
|---|---|
| `remember` | Store a memory under governance (injection quarantine, sensitivity, provenance, scope). Returns whether it was quarantined and why. |
| `recall` | Governed recall — returns a value or a safe abstention **with a reason** (never a low-confidence guess). |
| `forget` | Enforce erasure (GDPR Art. 17). Returns a tamper-evident **erasure certificate**. |
| `list_profiles` | List available compliance profiles and the tenant→profile map. |
| `audit_export` | Export the hash-chained governance audit trail for a tenant (`verified: true/false`). |
