...
| Opt-in BM25 + verbatim-turn indexing improves LoCoMo retrieval recall. | `--recall-boost` (`ranking.py`, verbatim indexing), `docs/locomo_results.md` | Internal local text-only evaluation, dev+test splits | Yes, local only | High | Supported narrowly: top-k evidence hit `0.137->0.297` dev / `0.115->0.268` test (+116%/+134%), generalizes to held-out split; off by default |
| The retrieval recall boost improves end-to-end LoCoMo accuracy. | Dev/test measurements | Internal local text-only evaluation | Yes, local only | High | Rejected: accuracy is flat (`0.207` dev / `0.172` test), still below `no_memory`; keyless extractive answer synthesis is the wall, not retrieval |
| A keyless extractive answer synthesizer exists (span extraction from evidence turns). | `src/cognitive_memory/answer.py`, `--answer-mode synthesis`, `tests/test_answer_synthesis.py` | Code/tests | Yes, local only | Medium | Supported as a component: returns concise question-typed spans; does not beat the abstain baseline end-to-end without an LLM answerer |
| Engram's memory now matches its compliance layer in end-to-end LoCoMo accuracy. | None | Unsupported | No | High | Rejected; the measured win is retrieval recall (roughly doubled), not end-to-end accuracy, which is capped by keyless answer synthesis |
